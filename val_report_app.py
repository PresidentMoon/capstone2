...
with tab2:
    ...
    sig_map = {
        ...
        '전체 시그널 보기':                 list(MATRIX_MAP.values().__class__),
    }
    target_sigs = sig_map.get(sig_choice, list({v[0] for v in MATRIX_MAP.values()}))
    if sig_choice == '전체 시그널 보기':
        target_sigs = list({v[0] for v in MATRIX_MAP.values()})
    ...
    st.caption(f"검색 결과 {len(ddisp)}개 종목  |  재무 기준: {latest_year}년  |  주가 기준: 오늘  |  PBR·PER은 낮을수록 저평가")
    st.dataframe(ddisp.style.format({'가치점수':'{:.3f}','PBR':'{:.2f}','PER':'{:.1f}','EV/EBITDA':'{:.1f}'}, na_rep='-'),
...
with tab3:
    ...
    if sel_corp:
        ...
        per_hint = ('낮은 PER — 이익 대비 저평가' if pd.notna(per) and per<10
                    ...
                    else '적자 기업 (계산 불가)' if not pd.notna(per)
                    else '보통 수준')
        ...
        with r1:
            ...
            CLST_D = {'EHS':'고안정·저평가 그룹','FD':'재무위험 그룹','HS':'균형안정 그룹',
                      'LR':'전통 가치주 그룹','NG':'성장주 그룹','WG':'취약성장 그룹'}
